...
impl ATree {
    pub fn new_from_geometries(
        geos: Vec<Rc<dyn Geometric>>,
        region: Rectangle,
        nper_leaf: Option<usize>
    ) -> ATree {
    ...
    pub fn search(&self, region: impl Geometric) -> Option<Vec<Rc<dyn Geometric>>> {
        ...
        let overlaps = self.nodes.iter().filter_map(|node| node.search(&bbox));
    ...
    fn pack(&mut self) -> () {
        ...
        let slices =
            (((bboxes.len() as f64) / (self.nper_leaf as f64)).sqrt().floor() as usize) + 1;
    ...
    fn pack_slice(&mut self, slice: &mut [Rectangle]) -> Node {
        ...
        for i in 0..nnodes {
            ...
            let leaf_geometry = Rectangle::new(
                x_min,
                geos[0].center().y(),
                x_max,
                geos[geos.len() - 1].center().y(),
                None
            );

...
impl Sky {
    ...
    fn reflect_tile(&self, tile: &Rectangle) -> Option<Vec<Rectangle>> {
        ...
        for (i, &n) in needs_reflection.iter().enumerate() {
            if n {
                ...
                new_bounds[reflection_bound] =
                    new_bounds[i] - tile.bounds()[i] + tile.bounds()[reflection_bound];
                let obounda = (i + 1) % 4;
    ...
    fn reflect_tile_over_corners(
        ...
        for (i, wd) in self.region.bounds().windows(2).enumerate() {
            let sqd =
                (wd[0] - tile.bounds()[i]).powf(2.0) + (wd[1] - tile.bounds()[i + 1]).powf(2.0);
            let distance = sqd.sqrt();
            needs_reflection[i] = distance < self.max_border;
        }
        let sqd =
            (self_bounds[0] - other_bounds[0]).powf(2.0) +
            (self_bounds[3] - other_bounds[3]).powf(2.0);
        let distance = sqd.sqrt();
